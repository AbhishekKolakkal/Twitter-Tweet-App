...
function resetCounter () {
  PropertiesService.getScriptProperties().setProperty('counter', '0');
  SpreadsheetApp.getActiveSpreadsheet().getSheetByName("Setup").getRange('b61').setValue(PropertiesService.getScriptProperties().getProperty('counter'));  
}
...
var value = parseInt(PropertiesService.getScriptProperties().getProperty('counter'));
  if (typeof count !== 'undefined'){
   var quota = count; 
  }else{
   var quota = 1; 
  ...
  var rows = SpreadsheetApp.getActiveSpreadsheet().getSheetByName("Select Tweets").getDataRange();
  var values = rows.getValues(); // An array of all tweets in ascending order
  var numRows = rows.getNumRows(); //15 this is the number of rows in the tweet
  var numCols = rows.getNumColumns(); // Undefined
  
  var list = new Array();
  
  for (var r = 4; r < numRows; r++){ // 
    
    var thisRow = values[r]; // this will contain the last tweet
    ...
    for (var c = 1; c < numCols; c++){
      ...
      if (typeof thisRow[c] !== 'undefined'){
        ...
        list[c].push(thisRow[c]); // this will have all the value of the tweet
      }
...
//        var word = list[k][Math.floor(Math.random()*(len + 1))]; // need to check this is random or not
        var word = list[k][value];
    ...
    PropertiesService.getScriptProperties().setProperty('counter', value);
    SpreadsheetApp.getActiveSpreadsheet().getSheetByName("Setup").getRange('b61').setValue(value);
    return tweet;    
  }
}
...
function onOpen() {
  ...
  ui.createMenu('Bot')
      ...
      .addItem('Reset Counter', 'resetCounter')
      .addToUi();
